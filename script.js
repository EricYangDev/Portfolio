...
function updateCenter(distance=Infinity){const locked=!!activeCategory;centerMark.classList.toggle('locked',locked);centerMark.classList.toggle('nudge',lockedAttempt);centerMark.classList.toggle('reset-ready',locked&&distance<110*layout.scale);centerTitle.textContent=locked?'Return to center':'Choose a domain';centerHint.textContent=locked?'to switch branches':'hover a large bubble'}
...
function showIntro(){panel.innerHTML='<div class="eyebrow">Hybrid hierarchy</div><h1>Eight-domain capability map</h1><p>The eight main domains stay circular. Primary skills and supporting capabilities use rounded cards so long labels remain readable without oversized bubbles.</p><div class="chips"><span class="chip">Circle domains</span><span class="chip">Card children</span><span class="chip">Rectangle collisions</span></div><div class="intro">Hover a domain to reveal its skills. Hover a skill to reveal its supporting capabilities. Return through the center before changing branches.</div>'}
function setPanel(){let key='intro';if(activeNode?.type==='mini')key='m:'+activeNode.mini;else if(activeNode?.type==='skill')key='s:'+activeNode.category+'/'+activeNode.skill;else if(activeNode?.type==='category')key='c:'+activeNode.category;if(key===lastPanel)return;lastPanel=key;if(activeNode?.type==='mini'){const n=activeNode;panel.innerHTML=`<div class="eyebrow">Supporting capability</div><div class="path">${esc(n.category)} / ${esc(n.skill)}</div><h1>${esc(n.mini)}</h1><p>This capability supports ${esc(n.skill)} within ${esc(n.category)}.</p><div class="chips"><span class="chip">Readable card</span><span class="chip">Branch locked</span></div>`}else if(activeNode?.type==='skill'){const n=activeNode;panel.innerHTML=`<div class="eyebrow">Primary skill</div><div class="path">${esc(n.category)} / ${esc(n.skill)}</div><h1>${esc(n.skill)}</h1><p>The skill card remains compact while its supporting capabilities fan outward as readable rounded cards.</p><div class="chips"><span class="chip">${n.minis.length} capabilities</span><span class="chip">Collision-safe</span></div><h2>Supporting capabilities</h2><div class="mini-list">${n.minis.map(x=>`<div class="mini-row">${esc(x)}</div>`).join('')}</div>`}else if(activeNode?.type==='category'){const n=activeNode,skills=Object.keys(SOURCE[n.category]),count=Object.values(SOURCE[n.category]).reduce((a,b)=>a+b.length,0);panel.innerHTML=`<div class="eyebrow">Capability domain</div><div class="path">Octagon / ${esc(n.category)}</div><h1>${esc(n.category)}</h1><p>This domain is locked for exploration. Return through the center hub before switching to another domain.</p><div class="chips"><span class="chip">${skills.length} skills</span><span class="chip">${count} capabilities</span></div><h2>Primary skills</h2><div class="mini-list">${skills.map(x=>`<div class="mini-row">${esc(x)}</div>`).join('')}</div>`}else showIntro()}
