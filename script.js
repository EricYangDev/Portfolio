...
let nodes=[],edges=[],pointer={x:-9999,y:-9999,inside:false},layout={w:0,h:0,baseCx:0,baseCy:0,cx:0,cy:0,coreRX:0,coreRY:0,interfaceScale:1},
...
function makeNode(label,type,ci,meta={}){
  ...
  if(labelLength>34) el.classList.add('label-xxlong');
  else if(labelLength>26) el.classList.add('label-xlong');
  else if(labelLength>18) el.classList.add('label-long');
...
function build(){
  ...
  const interfaceScale=clamp(Math.min(r.width/1120,r.height/690),.82,1.18);
  const coreBase=clamp(Math.min(r.width*.235,r.height*.32),170,270);
  const coreRX=clamp(coreBase*1.12,190,300);
  const coreRY=clamp(coreBase*.72,132,205);
  layout={
    w:r.width,
    h:r.height,
    baseCx:r.width*.5,
  ...
  stage.style.setProperty('--interface-scale',interfaceScale.toFixed(3));
...
function collisionRadius(n){
  ...
  if(n.type==='skill') return (n===activeNode ? 50 : 36)*scale;
  if(n.type==='mini') return (n===activeNode ? 78 : 37)*scale;
  return 0;
}
  ...
  for(let pass=0;pass<10;pass++){
    ...
    for(let i=0;i<visibleMinis.length;i++){
      for(let j=i+1;j<visibleMinis.length;j++){
        ...
        let d=Math.hypot(dx,dy);
        const needed=collisionRadius(a)+collisionRadius(b)+10*scale;
        if(d<needed){
          let ux,uy;
...
function computeTargets(){
  const scale=layout.interfaceScale||1;
  const activeIndex=activeCategory ? CATEGORY_KEYS.indexOf(activeCategory) : -1;
  const activeAngle=activeIndex>=0 ? -Math.PI/2 + activeIndex*Math.PI*2/CAT_COUNT : 0;
  const focusShift=activeCategory ? Math.min(86*scale,layout.h*.11) : 0;
  layout.cx=layout.baseCx-(Math.cos(activeAngle)*focusShift);
  layout.cy=layout.baseCy-(Math.sin(activeAngle)*focusShift);
  const effectiveCoreRX=activeCategory ? layout.coreRX*.82 : layout.coreRX;
  const effectiveCoreRY=activeCategory ? layout.coreRY*.82 : layout.coreRY;
  ...
  categories.forEach(n=>{
    n.angle=n.baseAngle;
    n.tx=layout.cx + Math.cos(n.baseAngle)*effectiveCoreRX;
    n.ty=layout.cy + Math.sin(n.baseAngle)*effectiveCoreRY;
  });
  ...
  CATEGORY_KEYS.forEach(category=>{
    ...
    let radius=(focused ? 132 : 82)*scale;
    if(activeSkill && activeSkill.category===category) radius+=8*scale;
    placeCircularGroup(group,cn.tx,cn.ty,cn.angle,radius,10*scale);
  });
  ...
  nodes.filter(n=>n.type==='skill').forEach(sn=>{
    ...
    const radius=(focused ? 120 : 42)*scale;
    const centerAngle=sn.branchAngle ?? categoryMap[sn.category].angle;
    placeCircularGroup(group,sn.tx,sn.ty,centerAngle,radius,8*scale);
  });
...
function renderedRadius(n){
  const base=(n.type==='category'?64:n.type==='skill'?33:31)*(layout.interfaceScale||1);
...
function updateCenterHub(centerDistance=Infinity){
  ...
  centerMark.classList.toggle('reset-ready',locked && centerDistance<122*(layout.interfaceScale||1));
  if(locked){
...
function determineFocus(){
  ...
  if(centerDistance<112*(layout.interfaceScale||1)){
    activeCategory=null;
    activeSkill=null;
    activeMini=null;
    activeNode=null;
...
function scheduleBuild(){
  ...
  rebuildTimer=window.setTimeout(build,90);
}
...
build(); updateHitState(); requestAnimationFrame(tick);
    ...
    const fallback = document.getElementById('skillMapFallback');
    if (fallback) fallback.remove();
  }

  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initCapabilityMap, { once: true });
  } else {
    initCapabilityMap();
  }
})();
